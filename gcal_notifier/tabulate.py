"""
This module is a part of the python-tabulate package.
It was done this way because a git version can't be published to pypi,
so in the future it'll be a normal dependency.
...
def tabulate(
    ...
    Variable `tabulate_formats` contains the list of current supported formats.
    ...
    with a ._repr_html_ method so that Jupyter Lab and Notebook display the
    HTML and a .str property so that the raw HTML remains accessible
    the unsafehtml table format can be used if an unescaped HTML format is
    required:
    ...
    >>> print(tabulate(
        [["spam$_9$", 41.9999], ["\\\\emph{eggs}", "451.0"]],
        tablefmt="latex_raw"))
    \\begin{tabular}{lr}
    \\hline
     spam$_9$    &  41.9999 \\\\
    ...
    >>> print(tabulate(
        [["spam", 41.9999], ["eggs", "451.0"]], tablefmt="latex_booktabs"))
    \\begin{tabular}{lr}
    \\toprule
     spam &  41.9999 \\\\
     eggs & 451      \\\\
    ...
    >>> print(tabulate(
        [["spam", 41.9999], ["eggs", "451.0"]], tablefmt="latex_longtable"))
    \\begin{longtable}{lr}
    \\hline
     spam &  41.9999 \\\\
     eggs & 451      \\\\
    ...
    ------------------------------------
    Tabulate will, by default, set the width of each column to the length of
    the longest element in that column. However, in situations where fields are
    expected to reasonably be too long to look good as a single line, tabulate
    can help automate word wrapping long fields for you. Use the parameter
    `maxcolwidth` to provide a list of maximal column widths

    >>> print(tabulate( \
          [('1', 'John Smith', \
            'This is a rather long description that might look better
                if it is wrapped a bit')], \
    ...
    cols = [
        ...
        for c, ct, fl_fmt, miss_v in zip(
            cols, coltypes, float_formats, missing_vals
        )
    ]
...
def _expand_numparse(disable_numparse, column_count):
    ...
    If `disable_numparse` is a list of indices, each of those indices areFalse,
...
def _format_table(fmt, headers, rows, colwidths, colaligns, is_multiline):
    ...
    if padded_rows and fmt.linebetweenrows and "linebetweenrows" not in hidden:
        ...
        append_row(
            lines, padded_rows[-1], padded_widths, colaligns, fmt.datarow
        )
    else:
...
class _CustomTextWrap(textwrap.TextWrapper):
    ...
    For the most part, the `_handle_long_word` and `_wrap_chunks` functionswere
    ...
    def _update_lines(self, lines, new_line):
        ...
        # Add color codes from earlier in the unwrapped line, and then track
        # any new ones we add.
        ...
        # still active, otherwise colors will bleed into other cells on the
        # console
...
def _main():
    ...
    -s REGEXP, --sep REGEXP   use custom column separator (default: whitespace)
...
def _pprint_file(fobject, headers, tablefmt, sep, floatfmt, file, colalign):
    ...
    print(
        tabulate(
            table, headers, tablefmt, floatfmt=floatfmt, colalign=colalign
        ),
        file=file,
    )
